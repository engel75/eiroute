...
- `{upstream_message}` — only used by `backend_bad_request`, which handles the
  "invalid JSON in request body" case generated by eiroute itself (not an
  upstream response)

Use simple string replacement, not a full templating engine.

Templates are used **only for errors that eiroute generates itself** (unknown
model, backend unavailable, semaphore timeout, stream interrupted, router
internal error, invalid request body). Errors that come from an upstream HTTP
4xx/5xx are handled differently — see "Error classification" below.
...
`param` is `null` for most errors; it is set to the offending field name (e.g.
`"messages"`, `"max_tokens"`) when the error can be attributed to a specific
request parameter.
...
5. Acquire the backend's semaphore with the configured `semaphore_timeout`. If
   the semaphore cannot be acquired in time, return `rate_limited`
...
upstream error classification applies (see "Error classification").
...
### Router-generated errors (template-based)

These errors are produced by eiroute itself before any upstream is contacted,
or when a transport failure occurs. They are rendered from the error template
file.

| Condition | Template | HTTP |
|---|---|---|
| `connection refused`, `no such host`, dial timeout, response header timeout | `backend_unavailable` | 503 |
| Unknown model in request | `unknown_model` | 404 |
| Semaphore acquire timeout | `rate_limited` | 429 |
| Mid-stream transport error | `stream_interrupted` | — |
| Invalid JSON in request body | `backend_bad_request` | 400 |
| Anything else (panic recovery, etc.) | `router_internal_error` | 500 |

### Upstream HTTP errors (passthrough + classification)

When the upstream returns HTTP 4xx or 5xx, eiroute does **not** use a
template. Instead it:

1. Reads the upstream response body (typically < 1 KB).
2. Extracts the error message verbatim — it accepts both the OpenAI-nested
   envelope (`{"error":{"message":"..."}}`) and the flat envelope used by
   sglang's chat/completions and embeddings endpoints
   (`{"object":"error","message":"...",...}`).
3. Classifies `type`, `code`, and `param` from the message content first
   (substring rules), falling back to the HTTP status.
4. Passes the upstream HTTP status through unchanged.
5. Wraps the result in the standard OpenAI error envelope.

Message-based classification rules (first match wins):

| Message contains | `type` | `code` | `param` |
|---|---|---|---|
| `"maximum context length"` or `"exceeds context length"` | `invalid_request_error` | `context_length_exceeded` | `"messages"` |
...
Status-based fallback (when no message rule matches):

| HTTP status | `type` | `code` |
|---|---|---|
| 400 | `invalid_request_error` | `upstream_bad_request` |
| 401 | `authentication_error` | `upstream_auth_failed` |
| 403 | `permission_error` | `upstream_forbidden` |
| 404 | `not_found_error` | `upstream_not_found` |
| 409 | `conflict_error` | `upstream_conflict` |
| 422 | `unprocessable_entity_error` | `upstream_unprocessable` |
| 429 | `rate_limit_error` | `backend_overloaded` |
| 501 | `invalid_request_error` | `not_implemented` |
| 5xx | `api_error` | `upstream_internal_error` |

If the upstream body is empty or not parseable JSON, a status-appropriate
fallback message is synthesised (e.g. `"The backend encountered an internal
error."`). The `param` field is `null` for all status-only fallbacks.
...
- Backend selection logic (unknown model, unhealthy backend, round-robin
  across duplicates).
- Semaphore acquire timeout returns `rate_limited` (429) with `Retry-After`.
- Upstream error classification: message-based rules and status fallbacks.
- `ParseUpstreamMessage` parses both nested and flat error envelopes.
...
- Drop the connection mid-stream — verify client receives SSE error event
  and `[DONE]`.
- Return HTTP 400 with nested OpenAI JSON body — verify `type`, `code`, and
  verbatim message are forwarded correctly.
- Return HTTP 400 with flat sglang JSON body and context-length message —
  verify `code: context_length_exceeded`, `param: messages`, verbatim message.
- Return HTTP 500 with no body — verify status 500 is passed through and a
  synthesised fallback message is returned.
